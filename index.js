...
async function checkParentCompletion(task) {
  ...
  if (updateErr) {
    ...
    console.log(`[A2A] Parent ${task.parent_task_id} → qa_testing (${succeeded.length} succeeded, ${failed.length} failed)`);
  }
}
    ...
    try {
      ...
      if (task.repo || task.project_id) {
        let query = supabase.from('agent_tasks')
          ...
          .in('status', ['qa_testing', 'completed'])
...
async function assignQueuedQATasks() {
  try {
    const { data: unassigned, error: taskErr } = await supabase
      .from("agent_tasks")
      .select("id, title, type, description, acceptance_criteria, priority, stage, parent_task_id, dispatched_by, repo, branch, context, result, assigned_agent")
      .eq("status", "qa_testing")
      .is("qa_agent", null)
  ...
  const channel = supabase
    ...
    .on(
      ...
      async (payload) => {
        ...
        if (task?.status === "qa_testing" && eventType === "UPDATE" && prev?.status !== "qa_testing" && task.stage) {
        ...
        // QA routing: when task moves to "qa_testing" with no qa_agent assigned, dispatch to QA worker
        // Agents now set qa_testing directly (no intermediate "done" status)
        if (task?.status === "qa_testing" && eventType === "UPDATE" && !task.qa_agent) {
          // Don't QA tasks that were already QA'd (prevent loops)
          if (task.type === "qa") return;
          // Skip if previous status was already qa_testing (e.g. qa_agent was just cleared for re-dispatch)
          if (prev?.status === "qa_testing") return;

          // Server-side guard: auto-skip QA for ops/review tasks without a PR
          ...
          console.log(`[QA] Task ${task.id} ("${task.title}") → qa_testing (unassigned), dispatching to QA worker`);
...
\`\`\``;
            ...
            if (betaAgent?.token) {
              ...
              if (resp.ok) {
                ...
                // Assign QA agent (task is already in qa_testing status)
                await supabase
                  .from("agent_tasks")
  ...
  try {
    ...
    for (const task of activeTasks_db) {
      ...
      const { data: freshTask } = await supabase
        ...
        .eq("id", task.id)
        .single();
      if (freshTask && ["qa_testing", "failed", "completed"].includes(freshTask.status)) {
        continue;
      }
